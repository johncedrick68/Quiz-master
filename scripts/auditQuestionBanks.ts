...
const visualPattern =
  /(\b(this|these)\b.*\b(sign|signal|traffic light|picture|image)\b|\b(sign|signal|traffic light|picture|image)\b.*\b(this|these)\b|ano.*\b(senyas|ilaw|larawan)\b.*\bito\b|\b(senyas|ilaw|larawan)\b.*\bito\b|alin .*nakalarawan|saan .*senyas.*ito)/i;
...
for (const [bank, questions] of Object.entries(banks)) {
  questions.forEach((question, index) => {
    ...
    if (
      ...
      problems.push(`${bank} #${index + 1}: invalid correctIndex`);
    if (
      new Set(question.options.map(normalize)).size !== question.options.length
    )
      problems.push(
        `${bank} #${index + 1}: duplicate/synonymous exact options`,
      );
    if (question.image) {
      ...
      if (!fs.existsSync(imagePath))
        problems.push(`${bank} #${index + 1}: missing image ${question.image}`);
    }
    const combinedText = `${question.question} ${question.options.join(" ")}`;
    if (
      /visual option in video|tungkol sa traffic sign|kaunting putol sa audio|ayon sa video|\b[ABC]\s*\(ang\b/i.test(
        combinedText,
      )
    ) {
...
const visualWithoutImage = allQuestions.filter(
  ({ question }) => visualPattern.test(question.question) && !question.image,
);
  ...
  JSON.stringify(
    {
      bankCounts: Object.fromEntries(
        Object.entries(banks).map(([name, questions]) => [
          name,
          questions.length,
        ]),
      ),
