...
export default function Home() {
  ...
  const begin = () => {
    setQuestions(shuffleArray([...motorcycleAA1English, ...motorcycleAA1EnglishPart2]).slice(0, QUESTION_COUNT));
  ...
  return <><Head><title>LTO Driving License Reviewer</title><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1" /></Head>
    <main className="min-h-screen bg-[#0649ad] text-white" style={{ backgroundImage: `linear-gradient(rgba(4,49,132,.62),rgba(4,49,132,.78)),url('${background}')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
      ...
      {screen === 'result' && <Results score={score} total={questions.length} onRetry={begin} onHome={() => setScreen('home')} />}
    </main>
  </>;
}
...
function Selection({ onStart, onBack }: { onStart: () => void; onBack: () => void }) { return <section className="mx-auto flex min-h-[calc(100vh-76px)] max-w-5xl items-center px-4 py-7 sm:py-10"><div className="lto-view w-full rounded-3xl border border-white/30 bg-[#064cac]/90 p-5 shadow-2xl backdrop-blur sm:p-10"><button onClick={onBack} className="mb-6 text-sm font-medium text-blue-100 transition hover:text-white">← Back to home</button><div className="mb-8 text-center"><p className="text-sm font-medium tracking-wide text-blue-200">Choose a reviewer</p><h2 className="mt-2 text-3xl font-semibold sm:text-5xl">Start your practice</h2></div><div className="grid gap-4 md:grid-cols-2"><button onClick={onStart} className="lto-start-card group rounded-2xl border-2 border-white bg-white p-6 text-left text-slate-900 shadow-lg"><span className="inline-flex rounded-full bg-[#064cac] px-3 py-1 text-xs font-semibold text-white">Available now</span><h3 className="mt-4 text-2xl font-semibold">Motorcycle</h3><p className="mt-1 text-lg font-semibold text-[#064cac]">Code A / A1</p><p className="mt-3 text-sm leading-relaxed text-slate-600">English · 60 random questions · Passing score: 48</p><span className="lto-start-button mt-5 inline-flex items-center gap-2 rounded-lg px-5 py-2.5 font-semibold text-white">Start reviewer <span className="text-lg">→</span></span></button><ComingSoon title="Motorcycle" subtitle="Code A / A1" detail="Tagalog questionnaire" /><ComingSoon title="Light Vehicle" subtitle="Code B, B1, B2" detail="English questionnaire" /><ComingSoon title="Light Vehicle" subtitle="Code B, B1, B2" detail="Tagalog questionnaire" /></div></div></section>; }
...
function Results({ score, total, onRetry, onHome }: { score: number; total: number; onRetry: () => void; onHome: () => void }) { const passed = score >= PASSING_SCORE; return <section className="mx-auto flex min-h-[calc(100vh-76px)] max-w-3xl items-center px-4 py-10"><div className="lto-view w-full rounded-2xl bg-white p-8 text-center text-slate-900 shadow-2xl sm:p-12"><img src="/images/lto_logo.webp" alt="LTO logo" className="mx-auto h-20 w-20 object-contain" /><p className="mt-4 text-sm font-semibold tracking-wide text-[#0649ad]">Reviewer complete</p><h2 className="mt-2 text-5xl font-semibold">{score} / {total}</h2><p className={`mt-3 text-2xl font-semibold ${passed ? 'text-green-600' : 'text-red-600'}`}>{passed ? 'Passed' : 'Needs more practice'}</p><p className="mt-2 text-lg text-slate-600">Passing score: {PASSING_SCORE} / {total} · You answered {Math.round((score / total) * 100)}% correctly.</p><div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><button onClick={onRetry} className="lto-primary rounded-lg px-6 py-3 font-semibold text-white">Try another set</button><button onClick={onHome} className="rounded-lg border-2 border-[#084daf] px-6 py-3 font-semibold text-[#084daf] transition hover:bg-blue-50">Back to home</button></div></div></section>; }
