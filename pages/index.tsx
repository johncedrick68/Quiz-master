...
export default function Home() {
  ...
  const [quizQuestions, setQuizQuestions] = useState<Question[]>(motorcycleAA1English);
  const [selected, setSelected] = useState<number | null>(null);
  const [timeLeft, setTimeLeft] = useState(QUESTION_SECONDS);
  const question = quizQuestions[current];
  const score = answers.reduce<number>((total, answer, index) => total + (answer === quizQuestions[index].correctIndex ? 1 : 0), 0);
  ...
  }, [screen, timeLeft, current, selected, quizQuestions.length]);

  const start = () => { setQuizQuestions(shuffleArray([...motorcycleAA1English, ...motorcycleAA1EnglishPart2]).slice(0, 60)); setCurrent(0); setAnswers([]); setSelected(null); setTimeLeft(QUESTION_SECONDS); setScreen('quiz'); };
  const advance = () => {
    if (selected === null) return;
    setAnswers((previous) => [...previous, selected]);
    if (current === quizQuestions.length - 1) setScreen('result');
  ...
  return <><Head><title>LTO Driving License Reviewer</title><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1" /></Head>
    <main className="min-h-screen bg-[#0649ad] text-white" style={{ backgroundImage: `linear-gradient(rgba(4, 49, 132, .62), rgba(4, 49, 132, .78)), url('${background}')`, backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
      ...
      {screen === 'quiz' && <Quiz question={question} current={current} total={quizQuestions.length} selected={selected} timeLeft={timeLeft} onSelect={setSelected} onNext={advance} onExit={() => setScreen('selection')} />}
      {screen === 'result' && <Result score={score} total={quizQuestions.length} onRetake={start} onHome={() => setScreen('home')} />}
    </main>
  </>;
}

function Header() {
  return <header className="border-b border-blue-300/25 bg-[#0348ac]/95 shadow-lg"><div className="mx-auto flex max-w-7xl items-center px-4 py-3 sm:px-7"><div className="flex min-w-0 items-center gap-3"><img src="/images/lto_logo.webp" alt="LTO logo" className="h-10 w-10 shrink-0 object-contain sm:h-12 sm:w-12" /><div className="min-w-0"><p className="truncate text-base font-semibold tracking-tight sm:text-xl">Driving License Reviewer</p><p className="mt-0.5 text-[10px] font-medium tracking-wide text-blue-200 sm:text-xs">Practice for Code A and A1</p></div></div></div></header>;
}

function Welcome({ onStart }: { onStart: () => void }) {
  return <section className="mx-auto flex min-h-[calc(100vh-76px)] max-w-6xl items-center px-4 py-6 sm:py-10"><div className="lto-enter w-full overflow-hidden rounded-3xl border border-white/30 bg-[#0b55b8]/85 shadow-2xl backdrop-blur-sm"><div className="flex min-h-[560px] flex-col items-center justify-center px-6 py-12 text-center sm:px-12"><p className="text-sm font-medium tracking-wide text-blue-100 sm:text-lg">Welcome to</p><img src="/images/lto_logo.webp" alt="Land Transportation Office" className="lto-float my-5 h-28 w-28 object-contain drop-shadow-lg sm:h-36 sm:w-36" /><h2 className="text-4xl font-semibold tracking-tight sm:text-6xl">LTO Driving License Reviewer</h2><p className="mt-5 max-w-2xl text-base leading-relaxed text-blue-50 sm:text-xl">Take a calm, focused practice test for motorcycle driving rules, safety, and road signs.</p><button onClick={onStart} className="mt-9 min-h-12 rounded-xl bg-[#ef0017] px-9 py-3 text-base font-semibold text-white shadow-lg transition duration-200 hover:-translate-y-0.5 hover:bg-[#c90013] focus:outline-none focus:ring-4 focus:ring-red-300/50">Continue</button><p className="mt-8 max-w-xl text-xs leading-relaxed text-blue-100">Independent study tool. This is not the official LTMS portal or an official LTO examination.</p></div></div></section>;
}
...
function Quiz({ question, current, total, selected, timeLeft, onSelect, onNext, onExit }: { question: Question; current: number; total: number; selected: number | null; timeLeft: number; onSelect: (index: number) => void; onNext: () => void; onExit: () => void }) {
  const minutes = Math.floor(timeLeft / 60); const seconds = `${timeLeft % 60}`.padStart(2, '0');
  return <section className="mx-auto max-w-6xl px-3 py-5 sm:px-6 sm:py-8"><div className="overflow-hidden rounded-xl border border-blue-100 bg-white text-slate-900 shadow-2xl"><div className="flex items-center justify-between gap-3 bg-[#0649ad] px-4 py-3 text-white sm:px-7"><p className="font-bold">Questions: <span className="text-lg">{current + 1}</span> <span className="font-medium text-blue-100">out of</span> {total}</p><div className="flex items-center gap-3"><span className={`flex items-center gap-2 rounded-full px-3 py-1 font-bold ${timeLeft <= 10 ? 'animate-pulse bg-red-500' : 'bg-blue-800'}`}><ClockIcon />{minutes}:{seconds}</span><p className="hidden text-lg font-semibold sm:block">Code A, A1</p></div></div><div className="border-b border-slate-300 p-3 sm:p-5"><div className="min-h-28 border border-slate-300 p-4 sm:p-6"><span className="block text-sm text-slate-600">Question</span><h2 className="mt-1 text-2xl font-bold leading-tight sm:text-3xl">{question.question}</h2></div></div><div className={`grid ${question.image ? 'lg:grid-cols-[330px_1fr]' : ''}`}>{question.image && <div className="flex items-center justify-center border-b border-slate-300 bg-slate-50 p-6 lg:border-b-0 lg:border-r"><img src={question.image} alt="Question reference" className="max-h-64 max-w-full rounded-lg object-contain shadow-sm" /></div>}<div>{question.options.map((option, index) => <button key={option} onClick={() => onSelect(index)} className={`flex w-full items-center gap-5 border-b border-slate-300 px-5 py-5 text-left text-lg font-medium transition sm:px-10 sm:py-6 sm:text-2xl ${selected === index ? 'bg-[#c9e9a5] text-slate-900 ring-2 ring-inset ring-[#65a839]' : 'hover:bg-blue-50'}`}><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-lg font-black ${selected === index ? 'bg-[#5aa431] text-white' : 'bg-slate-100 text-slate-700'}`}>{letters[index]}</span><span>{option}</span></button>)}</div></div><div className="flex items-center justify-between gap-4 p-4 sm:px-8"><button onClick={onExit} className="font-semibold text-slate-500 hover:text-[#0649ad]">Exit reviewer</button><button onClick={onNext} disabled={selected === null} className="min-h-11 rounded-lg bg-[#084daf] px-6 py-2.5 text-base font-bold text-white shadow transition hover:bg-[#063e91] disabled:cursor-not-allowed disabled:bg-slate-300">{current === total - 1 ? 'Finish' : 'Next'} →</button></div></div></section>;
}

function Result({ score, total, onRetake, onHome }: { score: number; total: number; onRetake: () => void; onHome: () => void }) { const passed = score >= 48; return <section className="mx-auto flex min-h-[calc(100vh-90px)] max-w-3xl items-center px-4 py-10"><div className="w-full rounded-2xl bg-white p-8 text-center text-slate-900 shadow-2xl sm:p-12"><img src="/images/lto_logo.webp" alt="LTO logo" className="mx-auto h-20 w-20 object-contain" /><p className="mt-4 text-sm font-bold uppercase tracking-widest text-[#0649ad]">Reviewer complete</p><h2 className="mt-2 text-5xl font-black">{score} / {total}</h2><p className={`mt-3 text-2xl font-black ${passed ? 'text-green-600' : 'text-red-600'}`}>{passed ? 'PASSED' : 'NEEDS MORE PRACTICE'}</p><p className="mt-2 text-lg text-slate-600">Passing score: 48 / {total} · You answered {Math.round((score / total) * 100)}% correctly.</p><div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row"><button onClick={onRetake} className="rounded-lg bg-[#084daf] px-6 py-3 font-bold text-white">Try another random set</button><button onClick={onHome} className="rounded-lg border-2 border-[#084daf] px-6 py-3 font-bold text-[#084daf]">Back to home</button></div></div></section>; }
